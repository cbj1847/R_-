...
# 양 끝값의 변화에 따른 평균의 변화는 크고, 중앙값의 변화는 없음
...
sd(height)                  # 표본표준편차 - 4.32

### 사분위수
quantile(cafe$Coffees, na.rm=T)
qs = quantile(cafe$Coffees, na.rm=T)
bp = boxplot(cafe$Coffees, main='커피 판매량 BoxPlot')
...
lowerlimit = qs[1] - 1.5*iqr
cars$dist[(cars$dist > upperlimit) | (cars$dist < lowerlimit)]      # 이상치 / 120
