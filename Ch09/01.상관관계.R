...
c(f.sd, s.sd)
cor.fs = cov.fs / (f.sd * s.sd)                 # 0.3913174
...
### 엄마와 딸의 관계
hf.dt = hf[hf$Gender == 'F', ]
hf.dt = hf.dt[c('Mother', 'Height')]
hf.dt = hf.dt[c('Mother', 'Height')] * 2.54
head(hf.dt)
cov(hf.dt$Mother, hf.dt$Height)                 # 10.92328
cor(hf.dt$Mother, hf.dt$Height)                 # 0.3136984

### 그림으로 비교
par(mfrow=c(1,2))
plot(hf.son$Father, hf.son$Height, main='Father vs Son')
plot(hf.dt$Mother, hf.dt$Height, main='Mother vs daugther')
par(mfrow=c(1,1))
